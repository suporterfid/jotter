...
PR4's read-only `GET /api/workspaces/{workspace}/search?q=` endpoint executes a parameter-bound MySQL FULLTEXT query scoped by `workspace_id`, ranks by the returned relevance score, and returns only note metadata plus a bounded excerpt derived from the projection. The endpoint never reads vault files and never exposes `search_content`.

PR5's workspace-scoped notes API creates, reads, updates, and deletes vault files through `VaultStorage`; it does not accept an absolute path or access the filesystem directly. Create and update project metadata incrementally, while a single-note read returns the current Markdown file contents rather than a MySQL body. Route note IDs are always resolved through the route workspace so cross-workspace IDs are not visible. Its `workspace.authorization` middleware is deliberately fail-closed until PR7 adds local identity and membership enforcement. TODO(spec: PR5): confirm a future path rename/move contract; PR5 updates content at the existing path only.
