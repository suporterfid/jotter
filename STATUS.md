...
- **#146 — MCP `read_note` tool calls undefined `VaultStorage::getNoteContent()`.** Fatal error on every call; the actual method is `readContents()`. Found in the same audit. Open, next in the queue.
- **#147 — `GrandpaSSOnIdentityProvider` unconditionally creates every new SSO user as an admin.** `is_admin => true` is hardcoded for new SSO-created users, and a null `is_admin` also resolves to admin. Found in the same audit. Open, gated behind `AUTH_PROVIDER=grandpasson` (not the default) but a real privilege-escalation bug for any deployment that enables it.
- **#148 — `main` has no branch protection requiring green CI before merge.** This is exactly how #140 regressed silently after #49. Open.
...
3. **In progress: security/correctness audit findings #145–#148**, filed 2026-07-28. #145 fixed and merged (PR #149). #146 (MCP `read_note` fatal bug), #147 (GrandpaSSOn admin escalation), and #148 (branch protection) are next in the queue, being worked one PR at a time.
