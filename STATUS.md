...
4. Milestone A/B/C/D are all recorded complete, and CI is now green again — treat that claim as trustworthy for the current `main` HEAD, but note there is still no branch protection requiring green CI before merge, so this can regress silently again (as #140 did after #49).
5. **Visual identity (#96)** — cross-cutting presentation workstream adopting a shared dark/purple design system with semantic tokens, Open Sans, and WCAG 2.2 AA across the SPA, the Laravel shell, and the published static site. Recorded complete; now unblocked since item 1 is done.
