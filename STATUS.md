...
- **CI Status:** 🟢 Green on `main`. #140 and #142 fixed via PR #144 (`claude/fix-140-142-ci-and-dead-tests`), merged and confirmed on two green GitHub Actions runs, both issues closed.
...
- ~~**#147 — `GrandpaSSOnIdentityProvider` unconditionally creates every new SSO user as an admin.**~~ **Closed.** `is_admin => true` was hardcoded for new SSO-created users, and a null `is_admin` also resolved to admin — both now default to `false`. Gated behind `AUTH_PROVIDER=grandpasson` (not the default). While fixing this, found a separate, pre-existing bug: the AUTHSESSID-cookie branch's `WHERE expires_at > time()` compares a DATETIME/TIMESTAMP column against a raw Unix-epoch int, which MySQL casts to just its leading digits (the year) — this branch can never match against the real `sessions` schema and is effectively dead code today. Not fixed (separate concern, noted for a future follow-up if this stub adapter is ever made real); the regression test uses its own compatible test-only schema to exercise the `is_admin` logic directly. Confirmed both ways (revert reproduces admin escalation, fix passes). Merged via **PR #151**.
- **#148 — `main` has no branch protection requiring green CI before merge.** This is exactly how #140 regressed silently after #49. Open, next in the queue.
...
3. **In progress: security/correctness audit findings #145–#148**, filed 2026-07-28. #145, #146, #147 fixed and merged (PR #149, #150, #151). #148 (branch protection on `main`) is the last one, next in the queue.
