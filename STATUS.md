...
- **CI Status:** 🟢 Fix for #140/#142 in PR from `claude/fix-140-142-ci-and-dead-tests`, root cause of #140 confirmed and fixed and reproduced locally both ways (fails without the fix, passes with it) — awaiting the PR's GitHub Actions run for final confirmation (see §3, #140)
...
- **#140 — root cause confirmed and fixed: `docker/php/entrypoint.sh` never chowned the vault storage directory.** Read the actual failing GitHub Actions log for run [30386678242](https://github.com/suporterfid/jotter/actions/runs/30386678242) via `gh api .../actions/jobs/{id}/logs`: `notes.spec.ts` fails fast (~10-11s, matching the assertion's own timeout) on every attempt with `Error: element(s) not found` on `[data-testid="editor-title"]` — the note-create flow never opens the editor. The old `alert()` in `App.vue`'s createNote error path hid the real error from Playwright; replaced with a DOM-visible `data-testid="error-banner"`, plus inline console/network diagnostics added to `notes.spec.ts` and a Laravel-log dump added to `.github/workflows/ci.yml` on failure. The next CI run's diagnostics showed the real cause: `500 POST /api/workspaces/1/notes`, and `storage/logs/laravel.log` gave the exact exception — `mkdir(): Permission denied at app/Domain/Vault/VaultPathGuard.php:85`. `config/jotter.php`'s default `vault_path` is `storage_path('app/vaults/default')`, but `docker/php/entrypoint.sh`'s directory-ownership loop only covered `storage/app/private`, never `storage/app/vaults` — on a genuinely fresh checkout (no local artifacts from prior runs already present), `www-data` cannot create that directory under `storage/app`. This is why the failure would not reproduce locally at first: this box's `storage/app/vaults` already existed from many earlier local runs, world-writable. Reproduced faithfully by deleting `storage/app/vaults` and resetting `storage/app` to `755` (matching a fresh checkout) — confirmed the failure locally with the pre-fix `entrypoint.sh`, then confirmed `jt e2e` and `jt test` pass with the fix (`storage/app/vaults` added to the chown/mkdir loop). Fixed in `claude/fix-140-142-ci-and-dead-tests`; the PR's own GitHub Actions run is the final confirmation.
...
1. **Merge the `claude/fix-140-142-ci-and-dead-tests` PR once its GitHub Actions run confirms green** — this blocks everything else per §0.3. Root cause (missing `storage/app/vaults` ownership in `docker/php/entrypoint.sh`) is fixed and verified both ways locally (fails without the fix, passes with it); the PR's own CI run is the last confirmation step.
