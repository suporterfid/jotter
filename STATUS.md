...
- **CI Status:** 🟡 Fix for #140/#142 open in PR from `claude/fix-140-142-ci-and-dead-tests` — PHPUnit and Vitest pass locally and in Docker; the alert()-hiding bug in #140 is fixed, but the failure itself did not reproduce locally after 6 fresh (`down --volumes` + `up` + `e2e`) attempts matching CI's exact config (`JOTTER_CI=1`), so root cause is not independently confirmed — awaiting the PR's actual GitHub Actions run to verify green (see §3, #140)
...
- **#140 — investigated in a Docker-capable environment; the alert()-hiding bug is fixed, but the underlying flake did not reproduce locally.** Read the actual failing GitHub Actions log for run [30386678242](https://github.com/suporterfid/jotter/actions/runs/30386678242) via `gh api .../actions/jobs/{id}/logs`: all three attempts (original + 2 retries) fail identically and fast (~10-11s each, matching the test's own 10s assertion timeout) with `Error: element(s) not found` on `[data-testid="editor-title"]` — the note-create flow silently never opens the editor, on the very first (non-retry) attempt too, which rules out a stale-note-from-a-prior-run theory (writes are upserts by path, not unique-constrained, so a duplicate path would not error). This matches either (a) `createNote()` failing and hitting the old `alert()` (auto-dismissed by Playwright, invisible in the trace), or (b) the immediately-following `getNote()` in `loadActiveNote()` failing, which was *already* silent (`console.error` only, no user-visible surface, no alert at all). Both are now easier to diagnose: `alert()` at `App.vue:212-215` is replaced with a DOM-visible `data-testid="error-banner"` element. Reproduction attempted 6 times against a fully fresh environment matching CI's exact invocation (`JOTTER_CI=1 CI=true`, `docker compose down --volumes` before each `up`+`e2e`, and a CPU-throttle attempt) — all 6 passed locally, so the specific trigger (likely a cold-boot timing/resource difference specific to GitHub's runners) is not independently confirmed. NoteRevisions' synchronous write (the #49-era leading candidate) was code-reviewed again and shows no bug. The PR from `claude/fix-140-142-ci-and-dead-tests` needs an actual green (or, if still red, now-diagnosable-via-banner) GitHub Actions run before this is closed.
- ~~**`WorkspaceAuthorizationPlaceholder` is fully deleted from `app/`**~~ — fixed. The six dead `withoutMiddleware(WorkspaceAuthorizationPlaceholder::class)` calls in `tests/Feature/WorkspaceNotesApiTest.php` are removed (not replaced with the real middleware class): every test in that file already authenticates as an `is_admin` user via `LocalIdentityProvider`, and `LocalIdentityProvider::isAuthorizedForWorkspace()` short-circuits to `true` for admins regardless of membership, so no bypass was ever needed. Verified via `jt test` (98 PHPUnit, 20 Vitest, all pass). Tracked in **#142**.
...
1. **Merge the `claude/fix-140-142-ci-and-dead-tests` PR once its GitHub Actions run is actually green** — this blocks everything else per §0.3. If the run still fails, the new `error-banner` (replacing the invisible `alert()`) should surface the real cause directly in the Playwright trace for a fast follow-up fix.
