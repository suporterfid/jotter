...
- **CI red on `main`: #140.** Investigated in a Docker-capable environment (`claude/fix-140-142-ci-and-dead-tests`). The GitHub Actions log for the failing run shows `notes.spec.ts` fails fast and identically on all 3 attempts (original + 2 retries) with the editor never appearing — not a text-content mismatch. `App.vue`'s `alert()` on note-create failure (invisible to Playwright, auto-dismissed) is replaced with a DOM-visible `data-testid="error-banner"`. The specific trigger did not reproduce locally across 6 fresh-environment attempts matching CI's config exactly, so root cause is not independently confirmed pending the PR's actual CI run. See `STATUS.md` §3.
