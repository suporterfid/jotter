...
- ~~**Note deletion is immediate and unrecoverable (#350, S–M, P2).**~~ **Implementation complete on `codex/cf4-note-trash`; awaiting PR/merge.** Notes now use soft delete with a per-workspace `.trash/`, list/restore/permanent-delete API and UI, occupied-path protection, reindex exclusion, and bounded `vault:purge-trash` retention. After merge, #349 (page ACLs) is next.
