...
- **No external content embeds (#353, M, P3).** **Selected and planned next; unblocked by #352.** `BlockRegistry.php` is already a sanitizing declarative registry with an `embed` block, but for internal transclusion. Add external embeds as a registered block with a domain allowlist and iframe sandbox, agreeing across both render paths. Not a plugin marketplace — spec §3 N3. Plan: `docs/superpowers/plans/2026-08-18-external-content-embeds.md`. **#354 follows.**
