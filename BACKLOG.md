...
As of 2026-07-29, every previously-tracked Milestone is delivered (backend and UI) and closed. The open work in this file is the remaining Confluence-parity section (#351–#360, filed 2026-08-17) plus the one item under "Needs a decision".
...
- ~~**Note deletion is immediate and unrecoverable (#350, S–M, P2).**~~ **Shipped in PR #363.** Notes now use soft delete with a per-workspace `.trash/`, list/restore/permanent-delete API and UI, occupied-path protection, reindex exclusion, and bounded `vault:purge-trash` retention. **#351 (notifications + page watching) is next.**
- ~~**Notifications only ever fire for `@mentions` (#351, M, P2).**~~ **Implementation complete on `codex/cf5-notification-watching`; pending PR integration.** Added the event vocabulary (`note_commented`, `comment_reply`, `note_edited`, `note_moved`, `note_deleted`), explicit and auto page watching with persistent unwatch, ACL-aware recipient filtering, actor suppression, 60-second coalescing, reply participation, audit events, API/UI links, and regression coverage. Verification: 241 PHP feature tests and 564 Vitest tests pass; frontend build passes. **Unblocks #352 after merge.** Plan: `docs/superpowers/plans/2026-08-18-notification-watching.md`.
