...
As of 2026-08-19, every previously-tracked Milestone is delivered (backend and UI) and closed. The open work in this file is the remaining Confluence-parity section (#357–#360, filed 2026-08-17) plus the one item under "Needs a decision"; #355 shipped in PR #375, #356 shipped in PR #377, and #357 is the selected next implementation issue.
...
#347–#360. #347–#356 are shipped; #357 is selected next; #358–#360 remain sequenced behind it.
...
- **No usage analytics (#357, M, P3) — selected next implementation; issue remains open.** Only raw `audit_log` exists in a table. Plan: `docs/superpowers/plans/2026-08-19-jotter-usage-analytics.md`. Deliver a durable rollup table, bounded idempotent cron command, ACL-safe workspace analytics API/dashboard, and an opt-in/off-by-default note-view event; never aggregate dashboards live over `audit_log`, because the raw rows are pruned after 90 days.
