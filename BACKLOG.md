...
As of 2026-08-19, every previously-tracked Milestone is delivered (backend and UI) and closed. The open work in this file is the remaining Confluence-parity section (#355–#360, filed 2026-08-17) plus the one item under "Needs a decision"; #354 is shipped in PR #373 and #355 is the current implementation issue.
...
- **Public sharing is whole-workspace only (#355, M, P3).** **In implementation; issue remains open until merge.** Added opaque hash-only per-note tokens, optional expiry and revocation with 404 semantics, authenticated ACL/audit management, isolated published-page rendering, token-scoped registered attachments, and note-editor controls. Final verification and merge record remain before marking this shipped.
