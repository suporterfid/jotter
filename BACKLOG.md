...
- **No external content embeds (#353, M, P3).** **Implementation complete locally; PR pending.** Added the registered external-embed block, empty-by-default HTTPS domain allowlist, mirrored PHP/TypeScript URL policy, sandboxed iframe rendering in server/SPA previews, link-only published/WYSIWYG output, fence/raw-iframe safety, and slash-menu insertion. Verification passed for focused PHP/API/publish coverage, the full frontend suite, and the frontend build. The issue remains open until PR merge. Plan: `docs/superpowers/plans/2026-08-18-external-content-embeds.md`. **#354 follows after merge.**
