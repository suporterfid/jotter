...
As of 2026-08-19, every previously-tracked Milestone is delivered (backend and UI) and closed. The open work in this file is the remaining Confluence-parity section (#359–#360, filed 2026-08-17) plus the one item under "Needs a decision"; #355 shipped in PR #375, #356 shipped in PR #377, #357 shipped in PR #380, and #358 is implementation-complete on `codex/cf12-pane-scope` pending PR/merge.
...
#347–#360. #347–#357 are shipped; #358 is implementation-complete pending PR/merge; #359–#360 remain sequenced behind it.
...
- ~~**Public sharing is whole-workspace only (#355, M, P3).**~~ **Shipped in PR #375.** Added opaque hash-only per-note tokens, optional expiry and revocation with 404 semantics, authenticated ACL/audit management, isolated published-page rendering, token-scoped registered attachments, audit events, and note-editor controls. Verification: 330 PHP tests passed (1 skipped), 586 Vitest tests passed, frontend build, PHP lint, and `git diff --check` passed.
...
- ~~**Split-screen blocked by document-global DOM lookups (#358, L, P3).**~~ **Implementation complete on `codex/cf12-pane-scope`; PR/merge pending.** Renderer heading IDs are pane-unique, outline navigation is local to `MarkdownPreview`, `NoteEditor` no longer performs editor-global DOM lookups, and the true split view now persists independent tabs/active notes with keyboard and drag-to-split controls, isolated drawers, responsive layout, and a Playwright regression. Verification: 608 Vitest tests, frontend build, existing notes E2E, and split-screen E2E passed.
